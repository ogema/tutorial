...
public class StaticTableExample {
	public StaticTableExample(final WidgetPage<?> page) {
		 final TextField editNewProgramName = new TextField(page, "editNewProgramName");
	        ...
	        table2.setContent(1, 1, getHorizontalFlexBox(page, editNewProgramName, buttonSaveAsNewProgram, "twocolUpdate"));
	}
	
	public static Flexbox getHorizontalFlexBox(WidgetPage<?> page, OgemaWidget w1, OgemaWidget w2, String id) {
	...
	public static Flexbox getVerticalFlexBox(WidgetPage<?> page, OgemaWidget w1, OgemaWidget w2, String id) {
		Flexbox flex = new Flexbox(page, id, true);
		flex.setDefaultFlexDirection(FlexDirection.COLUMN);
		flex.addItem(w1, null).addItem(w2, null);
		flex.setJustifyContent(JustifyContent.SPACE_AROUND, null);
		flex.setDefaultFlexWrap(FlexWrap.NOWRAP);
		return flex;
